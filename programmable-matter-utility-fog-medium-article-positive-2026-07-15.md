...
And then there's **open air** — which I first called "the deepest surviving barrier," claiming that biological reconfiguration is *strictly aqueous* and the air-tolerant state (a spore) and the reconfigurable state are **disjoint**. Having been wrong twice, I kill-tested that too. **It's also over-stated** — for the third time by the same mechanism: it bundles three separate things water does and reports the hardest as if it governed all of them. Unpacking them (and this pass is primary-sourced):

- **Shape-change in dry air → the claim is simply false.** Spore-coated tapes (Chen & Sahin, *Nature Communications*, 2015) build a humidity-driven **piston engine and electricity generator** that do real work in open air — ~17 J/kg, lifting 50× their weight, reversible over a *million* cycles. Pinecones and wheat awns do the passive version. Air-tolerant and shape-*changing* are not disjoint.
- **Onboard chemistry in dry air → over-stated.** Enzymes work in near-anhydrous organic solvents on a single **bound-water monolayer** (Klibanov, *Nature*, 2001), and microbial metabolism persists down to water activity **~0.585** — very dry, but not submerged. It needs a hydration *film*, not a bath.
- **Diffusion-based coordination between many units → this survives, and only this.** A morphogen signal doesn't cross a dry gap. Multi-unit signalling genuinely needs a continuous aqueous phase.

So the real open-air wall isn't "water." It's *"how do many units signal each other without a shared liquid?"* — **which is the exact question the communication wall left us with.** The two surviving barriers are the same barrier: **dry, local, radio-free signalling.** Solve that one problem and both fall together. (The one thing biology *does* concede here: its air-tolerant state — the spore — is metabolically inert. It survives dry; it doesn't compute dry. The counterexamples above are engineered or passive, not fully autonomous dry organisms.)
...
- **Open air:** *Needs:* — after a third kill-test, this is **not** a separate barrier. Shape-change and single-unit chemistry both work in dry air (spore engines; enzymes on a bound-water monolayer; metabolism to water-activity ~0.585); only *diffusion-based multi-unit signalling* needs liquid. That merges it with the count barrier: both reduce to **dry, local, radio-free signalling** — the single sharpest open problem.
...
- **Open-air kill-test (Part F5):** Chen, Sahin et al., *Nature Communications* 6:7346 (2015) (spore evaporation engine, ~17 J/kg, does work in air) · Klibanov, *Nature* 409:241 (2001) (non-aqueous enzymology, bound-water monolayer) · Stevenson…Hallsworth, *Environ. Microbiol.* 19:687 (2017) (metabolism to water-activity ~0.585).
- Scripts (reproducible): `scripts/power_scaling_wall.py` (the original L² illustration) · **`scripts/biology_wall_falsification.py`** · **`scripts/comms_energy_wall.py`** · **`scripts/open_air_wall.py`** (the three kill-tests).
